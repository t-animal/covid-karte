...
async function loadAndDisplayCountryWideCasesAndDeathSums(asSecondary: boolean) {
    ...
    const casesElem = getElementOrThrow(`.cases-section ${asSecondary? '.daily-sum.secondary-info' : ''} span:first-of-type`);
    const deathsElem = getElementOrThrow(`.deaths-section ${asSecondary? '.daily-sum.secondary-info' : ''} span:first-of-type`);
